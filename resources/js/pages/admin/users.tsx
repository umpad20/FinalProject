...
import { Edit, MoreHorizontal, Plus, Search, Shield, ShieldOff, Trash2, UserPlus, X } from 'lucide-react';
        ...
        <AdminLayout title="User Management">
            ...
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Create and manage admin & customer accounts</p>
                </div>
            ...
            <Card className="mb-6">
                <CardContent className="flex flex-col gap-3 p-4 sm:flex-row">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant={roleFilter === 'customer' ? 'default' : 'outline'} size="sm" onClick={() => setRoleFilter('customer')}>Customers</Button>
                    </div>
                </CardContent>
            </Card>
                ...
                <CardContent className="p-0">
                    <Table>
                        ...
                        <TableBody>
                                ...
                                <TableRow key={user.id}>
                                    ...
                                    <TableCell className="text-center">
                                        <Badge variant={user.isAdmin ? 'default' : 'secondary'}>
                                            {user.email === 'admin@jaypee.com' ? 'Main Admin' : user.isAdmin ? 'Admin' : 'Customer'}
                                        </Badge>
                                    </TableCell>
                            ...
                            {filtered.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={4} className="py-12 text-center text-muted-foreground">No users found.</TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                    </Table>
            ...
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a new admin or customer account.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleCreate} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="create-email" type="email" value={createForm.data.email} onChange={(e) => createForm.setData('email', e.target.value)} required />
                            {createForm.errors.email && <p className="text-xs text-destructive">{createForm.errors.email}</p>}
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="create-password">Password</Label>
                            <Input id="create-password" type="password" value={createForm.data.password} onChange={(e) => createForm.setData('password', e.target.value)} required minLength={8} />
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <Switch id="create-admin" checked={createForm.data.is_admin} onCheckedChange={(v) => createForm.setData('is_admin', v)} />
                        </div>
                        <DialogFooter>
                ...
                <DialogContent>
                    ...
                    <form onSubmit={handleEdit} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="edit-password" type="password" value={editForm.data.password} onChange={(e) => editForm.setData('password', e.target.value)} minLength={8} placeholder="Leave blank to keep current" />
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {editUser?.email === 'admin@jaypee.com' ? 'Main admin cannot be demoted' : 'Grant full admin privileges'}
                                </p>
                            </div>
                            <Switch id="edit-admin" checked={editForm.data.is_admin} onCheckedChange={(v) => editForm.setData('is_admin', v)} disabled={editUser?.email === 'admin@jaypee.com'} />
                        </div>
                        <DialogFooter>
