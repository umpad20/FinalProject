...
export default function AdminReports({ stats, orderStatusCounts, paymentMethods, topProducts, categoryBreakdown }: Props) {
    const avgOrderValue = stats.totalOrders > 0 ? stats.totalRevenue / stats.totalOrders : 0;

    const totalStatusOrders = Object.values(orderStatusCounts).reduce((a, b) => a + b, 0);

    const statusEntries = Object.entries(orderStatusCounts).map(([status, count]) => ({
        status: status.charAt(0).toUpperCase() + status.slice(1),
        count,
        percentage: totalStatusOrders > 0 ? Math.round((count / totalStatusOrders) * 100) : 0,
        color: statusColorMap[status] ?? 'bg-gray-500',
    }));

    const totalPayments = Object.values(paymentMethods).reduce((a, b) => a + b, 0);
    const paymentEntries = Object.entries(paymentMethods).map(([method, count]) => ({
        method,
        count,
                    ...
                    <Card>
                        ...
                        <CardContent className="p-0">
                            {topProducts.length === 0 ? (
                                <p className="p-6 text-center text-muted-foreground">No sales data yet. Products will appear here after orders are placed.</p>
                            ) : (
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                    ...
                                    <TableBody>
                                        {topProducts.map((p, i) => (
                                            <TableRow key={p.name}>
                                                ...
                                                <TableCell>
                                                    <Progress value={(p.unitsSold / maxUnitsSold) * 100} className="h-2" />
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {categoryBreakdown.length === 0 ? (
                                <p className="text-center text-muted-foreground">No category data yet.</p>
                            ) : (
                        ...
                        <Card>
                            ...
                            <CardContent>
                                {paymentEntries.length === 0 ? (
                                    <p className="text-center text-muted-foreground">No payment data yet.</p>
                                ) : (
                                    <div className="space-y-4">
